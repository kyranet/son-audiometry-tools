...
try {
	if ($PSBoundParameters.ContainsKey('Files')) {
		...
		$Files = (Get-ChildItem "$RootFolder/src","$RootFolder/include","$RootFolder/tests","$RootFolder/samples" -Include ('*.h', '*.hpp', '*.c', '*.cpp') -Recurse -File | Select-Object -ExpandProperty FullName)
